...
export function getEntryPoints(
  ...
  const entryPoints = options.initialEntryPoints ? new Set(options.initialEntryPoints) : new Set<string>()
  const seenProjects = new Set<string>()

  const findEntryPoints = (projectName: string, tsConfigFileName?: string): void => {
    ...
    if (foundTsConfig) {
      const tsconfig = readJsonFile(path.join(project.data.root, foundTsConfig))
